...
int pins[] = { GPin, APin, BPin };
int notes[] = { NOTE_G3, NOTE_A3, NOTE_B3 };
...
void loop(){
  ...
  for (int pin = 0; pin < 3; pin++){
    ...
    if (readCapacitivePin(pins[pin]) > touchedCutoff) {
      digitalWrite(LEDPin, HIGH);
      tone(buzzer, notes[pin], 100);
  ...
  if ( (millis() % 100) == 0){
    ...
    Serial.println(readCapacitivePin(GPin));
    Serial.print("Capacitive Sensor on Pin 4 reads: ");
    Serial.println(readCapacitivePin(APin));
    Serial.print("Capacitive Sensor on Pin 6 reads: ");
    Serial.println(readCapacitivePin(BPin));
  }
}
